...
func main() {
	...
	useUnicast := flag.Bool("use-unicast", false, `use unicast instead of multicast for communication
		with other keepalived instances`)

	configMapName := flag.String("services-configmap", "openelb-system/openelb-vip-configmap",
	...
	proxyMode := flag.Bool("proxy-protocol-mode", false, `If true, it will use keepalived to announce the virtual
	...
	iface := flag.String("iface", "", `network interface to listen on. If undefined, the nodes
                 default interface will be used instead`)

	httpPort := flag.Int("http-port", 8080, `The HTTP port to use for health checks`)

	releaseVips := flag.Bool("release-vips", true, `add --release-vips to keepalived args`)

	flag.Parse()

	if *configMapName == "" {
		klog.Fatalf("Please specify --services-configmap")
	}

	if *httpPort < 0 || *httpPort > 65535 {
		klog.Fatalf("Invalid HTTP port %d, only values between 0 and 65535 are allowed.", httpPort)
	}

	if *vrid < 0 || *vrid > 255 {
		klog.Fatalf("Error using VRID %d, only values between 0 and 255 are allowed.", vrid)
	}

	if *useUnicast {
		klog.Info("keepalived will use unicast to sync the nodes")
	}
	...
	klog.Info("starting LVS configuration")
...
func createApiserverClient(apiserverHost string, kubeConfig string) (*kubernetes.Clientset, error) {
	...
	klog.Infof("Creating API server client for %s", cfg.Host)
...
func loadIPVModule() error {
	...
	if err != nil {
		klog.V(2).Infof("Error loading ip_vip: %s, %v", string(out), err)
		return err
	}
...
func resetIPVS() error {
	klog.Info("cleaning ipvs configuration")
	_, err := exec.New().Command("ipvsadm", "-C").CombinedOutput()
	if err != nil {
...
func copyHaproxyCfg() {
	data, err := os.ReadFile("/haproxy.cfg")
	if err != nil {
		klog.Fatalf("unexpected error reading haproxy.cfg: %v", err)
	}
	err = os.WriteFile("/etc/haproxy/haproxy.cfg", data, 0644)
	if err != nil {
		klog.Fatalf("unexpected error writing haproxy.cfg: %v", err)
	}
}
