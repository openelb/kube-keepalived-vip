...
import (
	...
	"github.com/openelb/kube-keepalived-vip/pkg/store"
	"github.com/openelb/kube-keepalived-vip/pkg/task"
...
func (ipvsc *ipvsControllerController) getEndpoints(
	...
	if err != nil {
		klog.Warningf("unexpected error getting service endpoints: %v", err)
		return []service{}
	}
...
func (ipvsc *ipvsControllerController) getServices(cfgMap *apiv1.ConfigMap) []vip {
	...
	for externalIP, nsSvcLvs := range cfgMap.Data {
		if nsSvcLvs == "" {
			...
			klog.V(2).Infof("Adding VIP only service: %v", externalIP)
			continue
		}
		...
		if err != nil {
			klog.Warningf("error getting service %v: %v", nsSvc, err)
			continue
		}

		if !svcExists {
			klog.Warningf("service %v not found", nsSvc)
			continue
		}
		...
		for _, servicePort := range s.Spec.Ports {
			...
			if len(ep) == 0 {
				klog.Warningf("no endpoints found for service %v, port %+v", s.Name, servicePort)
				continue
			}
			...
			klog.V(2).Infof("found service: %v:%v", s.Name, servicePort.Port)
		}
	}
...
func (ipvsc *ipvsControllerController) sync(key interface{}) error {
	...
	if err != nil {
		klog.Errorf("error reloading keepalived: %v", err)
	}
...
func (ipvsc *ipvsControllerController) Start() {
	...
	go func() {
		klog.Infof("Starting HTTP server on port %d", ipvsc.httpPort)
	...
	klog.Info("starting keepalived to announce VIPs")
...
func NewIPVSController(kubeClient *kubernetes.Clientset, namespace string, useUnicast bool, configMapName string, vrid int, proxyMode bool, iface string, httpPort int, releaseVips bool) *ipvsControllerController {
	...
	if err != nil {
		klog.Fatalf("Error getting POD information: %v", err)
	}

	pod, err := kubeClient.CoreV1().Pods(podInfo.Namespace).Get(context.Background(), podInfo.Name, metav1.GetOptions{})
	if err != nil {
		klog.Fatalf("Error getting %v: %v", podInfo.Name, err)
	}
	...
	if err != nil {
		klog.Fatalf("Error getting local IP from nodes in the cluster: %v", err)
	}
	...
	if iface == "" {
		...
		klog.Info("No interface was provided, proceeding with the node's default: ", iface)
	}
	execer := utilexec.New()
	...
	if err != nil {
		klog.Fatalf("Error loading templates: %v", err)
	}
	...
	if err != nil {
		klog.Fatalf("Error parsing configmap name: %v", err)
	}
	...
	ipvsc.mapLister.Store, ipvsc.mapController = cache.NewInformer(
		cache.NewListWatchFromClient(ipvsc.client.CoreV1().RESTClient(), "configmaps", cmns,
			fields.OneTermEqualSelector(ObjectNameField, cmn)),
	...
	http.HandleFunc("/health", func(rw http.ResponseWriter, req *http.Request) {
		...
		if err != nil {
			klog.Errorf("Health check unsuccessful: %v", err)
			http.Error(rw, fmt.Sprintf("keepalived not healthy: %v", err), 500)
			return
		}

		klog.V(3).Info("Health check successful")
		fmt.Fprint(rw, "OK")
	})
...
func (ipvsc *ipvsControllerController) getConfigMap(ns, name string) (*apiv1.ConfigMap, error) {
	//ipvsc.client.CoreV1().ConfigMaps(ns).Get(context.Background(),name,metav1.GetOptions{})
